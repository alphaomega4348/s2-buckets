...
export default function SustainabilityReportsSection() {
  ...
  const [flips] = useState(() =>
    CARDS.map((_, i) =>
      useSpring({ from: { opacity: 0, transform: "rotateX(-90deg)" }, to: { opacity: 1, transform: "rotateX(0deg)" }, delay: 500 + i * 200 })
    )
  );

  // image fade
  const imgFade = useSpring({ from: { opacity: 0, transform: "translateY(20px)" }, to: { opacity: 1, transform: "translateY(0)" }, delay: 1400 });
