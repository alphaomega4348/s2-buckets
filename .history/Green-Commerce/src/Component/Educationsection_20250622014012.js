...
const faqItems = [
  ...
  },
  {
    ...
    answer: `
  Eco Grade is a single‐letter sustainability rating (A*, A, B+, B or C) that summarizes a product’s overall environmental performance. We calculate it by evaluating a set of key metrics:
  
   • Plastic Reduction (%): How much plastic has been replaced or eliminated  
   • Chemical Usage (%): How low is the use of harmful chemicals  
   • CO₂ Emissions Reduction (%): How much carbon footprint is cut  
   • Recyclability (yes/no) & Packaging Recyclability (yes/no)  
   • Biodegradability (%): How quickly materials break down after use  
   • Water Consumption (liters) & Energy Consumption (kWh) per unit  
   • Product Weight (kg) & Expected Lifespan (years)
  
  Each metric is first normalized to a 0–100 scale, then weighted by its environmental impact and combined into a single composite score. Finally, that score is mapped to a letter grade:
  
   • **A\*** (≥ 90): Outstanding environmental performance  
   • **A** (80–89): Very good, with only minor areas to improve  
   • **B+** (70–79): Good, but some metrics need attention  
   • **B** (60–69): Fair, significant room for improvement  
   • **C** (< 60): Below standard, requires substantial changes  
  
  For example, a product that reduces 84% of its plastic, limits chemical use to 19%, cuts CO₂ emissions by 37%, is fully recyclable, achieves 60% biodegradability, uses only 2.5 L of water and 0.1 kWh of energy, weighs 0.75 kg, and lasts 2 years would score high enough to earn an **A** grade—letting you identify top‐performing eco‐friendly products at a glance.
    `.trim()
  }
