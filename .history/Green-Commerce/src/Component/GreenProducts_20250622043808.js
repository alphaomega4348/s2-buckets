...
import cleaningcloth from "../assets/green products/cleaning/cleaningcloth"
...
export default function GreenProducts({ description, topN = 5 }) {
    ...
    // build the data source: if searching, use recs mapped to full products
    const dataSource = description
            ...
            <main style={{ flex: 1 }}>
                ...
                >
                    ...
                    {visible
                        ...
                        .slice(0, 5)          // 👈 show only the first 5 matching products
                        .map(p => (
                    
                            <Link
                                key={p._id}        // you’re already using Mongo’s id in the URL
                                to={`/product/${p._id}`}
                                style={{ textDecoration: 'none', color: 'inherit' }}
                            >
                            {/* yahi pe likh do na logic image mein error yaya too yahi nhi chalgea  */}
                                <ProductCard p={p} />
                            </Link>
                        ))}
