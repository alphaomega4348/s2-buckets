              ...
              <div>
                ...
                <button
                  style={{
                    ...styles.button,
                    ...styles.btnApply,
                    animation: blinkCoupon ? 'blink 0.2s linear 2' : undefined,
                    outline: blinkCoupon ? '2px solid #ED64A6' : 'none',
                  }}
                  ref={couponSectionRef}
                  onClick={() => {
                    if (couponCode.trim()) {
                      // Example: 5% discount on the total MRP (after other discounts, or as per your logic)
                      const totalMRP = basket.reduce((sum, i) => sum + Number(i.price || 0), 0);
                      const couponDisc = Math.round((totalMRP * 5) / 100);
                      setCouponApplied(true);
                      setCouponDiscount(couponDisc);
                      SettotalPayment(totalMRP - couponDisc);
                    } else {
