...
export default function GreenProducts({ description, topN = 5 }) {
    ...
    const navItems = [
        ...
        { to: '/reports', label: 'Sustainable Reports', icon: '📊' },
      ];
    const sidebar = {
        width: '240px',
    ...
    // Timeline
    const timelineContainer = { position: 'relative', paddingLeft: '20px' };
    const lineStyle = {
        position: 'absolute',
        left: '9px',
        top: 0,
    ...
    const nodeWrapper = { position: 'relative', marginBottom: '1.5rem' };
    // **Now iconStyle is a function** that returns an object
    const iconStyle = (hovered) => ({
        width: '20px',
        height: '20px',
        borderRadius: '50%',
        background: 'white',
        ...
        <div>
            ...
            <div style={{
                ...
                <aside style={sidebar}>
                    ...
                    <div style={timelineContainer}>
                        ...
                        {navItems.map(({ to, label, icon }, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <Link to={to} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
                                        <span style={iconStyle(isHover)}>{icon}</span>
                                        <span style={labelCard(isHover)}>{label}</span>
                                    </Link>
                                </div>
                            );
                        })}
                    ...
                    {/* — Quick Picks Vertical Scroller — */}
                    <h4 style={quickTitle}>Quick Picks</h4>
                    <div style={quickWrap}>
                        {products.slice(0, 10).map(p => (
                            <img
                                ...
                                alt={p.productName}
                                style={thumbStyle}
                                onMouseEnter={e => Object.assign(e.currentTarget.style, thumbHover)}
                                onMouseLeave={e => Object.assign(e.currentTarget.style, thumbStyle)}
                            />
                        ))}
                    </div>
                </aside>
