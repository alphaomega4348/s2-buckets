...
export default function SustainabilityReportsSection() {
  ...
  // Panel springs: first slides in from left, second from right
  const panelSprings = useSprings(
    PANELS.length,
    PANELS.map((_, i) => ({
      from: { opacity: 0, transform: `translateX(${i === 0 ? -200 : 200}px)` },
  ...
  return (
    <div style={{
      background: "#faf7f7",import React from "react";
import { useSpring, animated, useTrail, useSprings } from "react-spring";
      ...
      <div style={{ display:"flex", gap:16, marginBottom:32 }}>
        {CARDS.map((c, i) => (
          <animated.div key={i} style={{
            ...
            <div style={{ marginTop:8, color:"#533A2B", fontWeight:600, fontSize:14 }}>
              {c.label}
            </div>
          </animated.div>
        ))}
