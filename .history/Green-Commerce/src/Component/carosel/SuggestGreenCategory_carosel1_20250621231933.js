...
export default function SuggestGreenCarousel() {
    ...
    useEffect(() => {
        intervalRef.current = setInterval(() => {
            setIndex(i => Math.min(i + 1, categories.length - visibleCount));
        }, 3000);
    ...
    // Styles
    const wrapper = { display: 'flex', alignItems: 'center', position: 'relative', margin: '24px 0' };
    const arrowStyle = {
    ...
    return (
        <div style={wrapper}>
            <button onClick={handlePrev} style={{ ...arrowStyle, marginRight: '8px' }} disabled={index === 0}>&lt;</button>
            <div style={viewport}>
                <div style={track}>
                    {categories.map((cat, i) => (
                        <div key={i} style={card} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
            ...
            <button onClick={handleNext} style={{ ...arrowStyle, marginLeft: '8px' }} disabled={index >= categories.length - visibleCount}>&gt;</button>
        </div>
    );
}