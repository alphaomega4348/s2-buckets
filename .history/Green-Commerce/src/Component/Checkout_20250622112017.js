...
function Checkout() {
  ...
  // Use basket index for selection mapping to support duplicates
  ...
  // Update selection mapping if basket changes
  React.useEffect(() => {
  ...
  // Correct selected count (by index)
  const itemsSelectedCount = Object.keys(selectedItems).filter(idx => selectedItems[idx]).length;

  // Remove by index, supports duplicates!
  const removeSelected = () => {
    // Remove from last to first to prevent index shift bugs
          ...
          <div style={styles.section}>
            ...
            <div>
              <div style={styles.summaryRow}>
                <span>Total MRP</span>
                <span>₹{basket.reduce((sum, i) => sum + Number(i.price || 0), 0)}</span>
              </div>
              <div style={styles.summaryRow}>
