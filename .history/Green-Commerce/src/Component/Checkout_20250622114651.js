...
export default function CheckoutProduct({
  ...
  stock = 5,
  originalPrice,
  discountPercent = originalPrice ? Math.round(100 - (price / originalPrice) * 100) : 0,
  returnWindow = "14 days",
  onRemove,
}) {
  // Defensive: fallback to 0 if rating missing, NaN, negative, etc.
  const safeRating = Math.max(0, Number.isFinite(Number(rating)) ? Number(rating) : 0);

  // Defensive: fallback image handling
